...
5.1 "Find More Companies" button (live pipeline trigger)

A button in the app that runs Mode B (the live Shodan pipeline) on demand and refreshes the list with the new companies.

Where it lives: top of the company list screen, next to the filters.

Flow when clicked:

User picks a target — a country and/or a product/keyword (small form or two dropdowns).
The app calls a backend endpoint (e.g. Next.js API route /api/find-companies) with that target.
The endpoint runs the pipeline: query the Shodan API for that slice → run the exact same cleaning stages 2–6 (extract, resolve, KEV join, score, exposure) → upsert the new companies into the Supabase companies and company_vulns tables.
The endpoint returns when done; the list re-queries Supabase and shows the new rows.

UX while it runs:

Show a loading state ("Searching Shodan and scoring companies…"). This can take 20–60 seconds.
Handle failure gracefully: if the Shodan call errors or rate-limits, show a clear message ("Couldn't fetch new data right now"), do NOT crash, and leave the existing list intact.
De-duplicate: upsert on company so re-running the same target updates rather than creates duplicates.

Guardrails / cost:

Cap how many records a single click pulls (Shodan credits are limited) — e.g. a fixed result limit per query.
The Layer-3 AI cleaning still only runs on ambiguous domains, with per-domain caching.

DEMO WARNING (do not skip): Do not click this button live in front of the interviewer unless it has been tested repeatedly and is reliably fast. It calls an external API and writes to the database while the audience waits — a stall or error mid-demo is worse than not showing it. Safe play: demo the app on pre-loaded data, explain that this button runs the live pipeline, and only click it live if you are certain it performs. Treat it as an "above and beyond" feature that exists and works, shown carefully
